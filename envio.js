...
// Botón para abrir el modal de productos desde el modal de pedido.
// Este botón se habilitará solo cuando exista un pedido (mPedido.value esté definido).
...
/**
 * Habilita o deshabilita el botón de productos según la existencia de
 * un número de pedido en el formulario del modal de pedido. Si no hay
 * un pedido asignado (p.ej. cuando se está creando uno nuevo y aún no
 * se ha guardado), el botón se deshabilita para evitar que se intente
 * agregar productos sin un pedido existente.
 */
function updateProductosButtonState() {
  if (!btnProductos) return;
  // Existe y tiene valor el input de pedido
  const hasPedido = mPedido && String(mPedido.value || "").trim() !== "";
  btnProductos.disabled = !hasPedido;
}
...
function openModal(row){
  ...
  // Habilitamos el botón de productos, ya que estamos editando un pedido
  // existente y existe un número de pedido asignado. Sin este llamado
  // el botón podría permanecer deshabilitado de ediciones previas.
  updateProductosButtonState();
...
btnNuevo.onclick = () => {
  ...
  // Al crear un nuevo pedido aún no existe un número asignado, por lo
  // que deshabilitamos el botón de productos hasta que se guarde
  // correctamente y se vuelva a abrir en modo edición.
  updateProductosButtonState();
...
window.onload = () => {
  ...
  // Al iniciar la aplicación deshabilitamos el botón de productos. Solo se
  // habilitará cuando se abra un pedido existente para editar. Esto evita
  // que se intente agregar productos a un pedido que aún no existe.
  updateProductosButtonState();
